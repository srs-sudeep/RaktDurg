...
export function SplashScreen({ message = "Loading RaktDurg…" }: SplashScreenProps) {
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-background via-card to-red-50">
      <div className="pointer-events-none absolute -left-32 top-0 h-96 w-96 rounded-full bg-red-200/30 blur-3xl" />
      <div className="pointer-events-none absolute -bottom-32 -right-32 h-96 w-96 rounded-full bg-primary/10/40 blur-3xl" />
      ...
      <div className="relative flex flex-col items-center">
        ...
        <h1 className="mt-6 text-3xl font-bold text-foreground">RaktDurg</h1>
        <p className="mt-1 text-sm text-muted-foreground">District Blood Bank Platform</p>
        <p className="mt-4 text-xs font-medium text-muted-foreground">By IBITF and IIT Bhilai</p>
        <p className="text-xs text-muted-foreground">Powered by Recogx Init</p>
...
export function PageLoader({ label = "Loading…" }: { label?: string }) {
  return (
    <div className="flex min-h-[50vh] flex-col items-center justify-center gap-5 py-16">
      <div className="relative">
        <div className="h-16 w-16 animate-spin rounded-full border-4 border-primary/15 border-t-red-600" />
        <img
          src="/logo.svg"
          alt=""
