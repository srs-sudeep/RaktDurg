...
export default function ProfilePage() {
  ...
  if (isLoading) return <p className="text-[13px] text-slate-500">Loading profile…</p>;
  if (error || !data) return <p className="text-[13px] text-red-600">Could not load profile.</p>;
  ...
  return (
    <div className="space-y-3">
      <PageHeader
        title="Profile"
        description="Your account, role, and session details."
        actions={
      ...
      <Panel title="Account">
        <dl className="grid gap-3 text-[13px] sm:grid-cols-2 lg:grid-cols-3">
          <div>
            <dt className="text-[11px] text-slate-500">Username</dt>
            <dd className="font-medium">{data.username}</dd>
          </div>
          <div>
            <dt className="text-[11px] text-slate-500">Display name</dt>
            <dd>{data.display_name || "—"}</dd>
          </div>
          <div>
            <dt className="text-[11px] text-slate-500">Role</dt>
          ...
          <div>
            <dt className="text-[11px] text-slate-500">Last login</dt>
      ...
      <Panel title="Preferences" description="Local display preference (admin can change role).">
        <form className="flex flex-wrap items-end gap-3" onSubmit={onSave}>
          <FormField label="Preferred display name" htmlFor="display_name" className="min-w-[240px]">
            <FormInput
              id="display_name"
